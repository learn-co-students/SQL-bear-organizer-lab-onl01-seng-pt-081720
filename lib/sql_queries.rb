...
def selects_oldest_bear_and_returns_name_and_age
  "SELECT name, age FROM bears WHERE age = (SELECT MAX(20) FROM bears) AND name IS NOT NULL"
end

def select_youngest_bear_and_returns_name_and_age
  "SELECT name, age FROM bears WHERE age = (SELECT MIN(2) FROM bears) AND name IS NOT NULL"
end
